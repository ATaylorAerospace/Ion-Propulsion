...
class KeplerLEOVelocity:

    def __init__(self, a, per, e=0, tau=0, Omega=0, w=0, i=0, ks=None):
        """Constructor for Keplerian orbital elements.

        The orbital elements defined this way do 
...
def phaseAngle(pos, los='-z'):
  """
    ...
    The phase angle is the angle between the star and the Planet (or Binary Star)
...
def calculate_phase_angle(los, pos):

  l = LineOfSight(los)
  
  if pos.shape == (3,):
    # Single position
    return np.arccos(np.dot(-pos, -l.los) / np.linalg.norm(pos)) / np.pi * 180

  else:
    # Multiple positions
    result = np.zeros(len(pos[:,0]))
    for i in range(len(pos[:,0])):
      result[i] = np.arccos(np.dot(-pos[i], -l.los) / np.linalg.norm(pos[i]))
    return result / np.pi * 18
...
M = (2/3)*(5 x 10^12)(3.1 x 10^7)(1.5 x 10^8)(1/2)^2
